...
@section('content')
    <form action="{{ route('anggota.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select id="id_jenis_anggota" name="id_jenis_anggota" class="mt-1 block w-full border p-2 rounded {{ $errors->has('id_jenis_anggota') ? 'border-red-500' : 'border-gray-300' }}" required>
                ...
                @foreach ($jenisAnggotas as $jenisAnggota)
                    <option value="{{ $jenisAnggota->id_jenis_anggota }}" {{ old('id_jenis_anggota') == $jenisAnggota->id_jenis_anggota ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="date" id="tgl_daftar" name="tgl_daftar" value="{{ old('tgl_daftar') }}" 
        ...
        <div class="mb-4">
            ...
            <input type="date" id="masa_aktif" name="masa_aktif" value="{{ old('masa_aktif') }}" 
        ...
        <div class="mb-4">
            ...
            <select id="fa" name="fa" class="mt-1 block w-full border p-2 rounded {{ $errors->has('fa') ? 'border-red-500' : 'border-gray-300' }}" required>
                <option value="Y" {{ old('fa') == 'Y' ? 'selected' : '' }}>Ya</option>
                <option value="T" {{ old('fa') == 'T' ? 'selected' : '' }}>Tidak</option>
            </select>
            @error('fa')
                <div class="text-red-500 text-sm">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-4">
            ...
            <input type="file" id="foto" name="foto" 
                   class="mt-1 block w-full border p-2 rounded {{ $errors->has('foto') ? 'border-red-500' : 'border-gray-300' }}" required>
            @error('foto')
                <div class="text-red-500 text-sm">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="password" id="password" name="password" value="{{ old('password') }}" 
                   class="mt-1 block w-full border p-2 rounded {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }}" required>
            @error('password')
                <div class="text-red-500 text-sm">{{ $message }}</div>
            @enderror
        </div>
